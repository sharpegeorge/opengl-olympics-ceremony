...
def load_vertices_from_obj(file_path):
    """
    Extracts a list of vertices from an .obj file.
    
    Parameters:
    - file_path (str): The path to the .obj file.

    Returns:
    - vertices (list of list of float): A list where each element is a list representing
                                        a vertex with x, y, z coordinates.
    """
    vertices = []
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if parts and parts[0] == 'v':
                    # Convert the vertex components to floats and add to vertices list
    ...
    print(f"Loaded {len(vertices)} vertices from '{file_path}'")
    return vertices

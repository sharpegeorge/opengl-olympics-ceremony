...
class MaxBrightnessShader(BaseShaderProgram):
    # Renders with max brightness (acts like a fake light)

    def __init__(self):
        # Initialize without file paths by passing None for the name
        super().__init__(name=None)

        # Manually set the vertex and fragment shader sources
        self.vertex_shader_source = '''
            #version 130
            in vec3 position;   // vertex position
            uniform mat4 PVM;    // Perspective-View-Model matrix

            void main() {
                gl_Position = PVM * vec4(position, 1.0);  // Transform position using PVM matrix
            }
        '''
        self.fragment_shader_source = '''
            #version 130
            void main() {                   
                gl_FragColor = vec4(1.0, 0.6, 0.3, 1.0);  // Warm orange tone
            }
        '''

        # Initialize the PVM uniform to store the projection-view-model matrix
        self.uniforms['PVM'] = Uniform('PVM')
